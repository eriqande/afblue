##New Random:  separate sexes with random mating; true replication of P; skip Fst and Ne
Ne = 40
NLoci = 100
S = 100  # Random, Ne = 40, S = 100 = Scenarios S-V
NGens = 10
NReps = 100
MaxSib = seq(1:NReps)
MaxFamily = 9 # 1 = Wright-Fisher; > 1 = overdispersion
Familysize = seq(1:MaxFamily)
Sibcheck = 0 # 0 purges FS+HS; 1 purges only HS
SSP = rep(0,NLoci)
PBar = rep(0,NLoci)
RPP = rep(0,NLoci)
PBar1 = rep(0,NLoci)
...
Separate <- function(parents)           { ## This is separate sexes with random mating
 ...
 RanFamily <- function(parents)           {  ## simulates production of large half-sib families
    # N = number of inds (rows)
    N = dim(parents)[[1]]
    Half = N/2
    Males = seq(1:Half)
...
###########################
## Produce a 0sample from the final generation 
Offspring = RanFamily(Parents1) 
...
AllS = c(HMS, HMS1, HMS2, HMS3, HMS4, HMS5, HMS6)
...
AllESS = c(ESS, ESS1, ESS2, ESS3, ESS4, ESS5, ESS6)
...
AllRRMSE = c(RRMSE,ARRMSE,BRRMSE,CRRMSE,DRRMSE,ERRMSE,FRRMSE)
