##New Mixed:  separate sexes with mixed matings; true replication of P; skip Fst and Ne
Ne = 500
NLoci = 100
S = 40
NGens = 10
...
MaxSib = seq(1:NReps)
MaxFamily = 30
Sibcheck = 0  # 0 purges all; 1 purges only HS
ProbFamily = 0.1
Familysize = seq(2:MaxFamily)+1
SSP = rep(0,NLoci)
PBar = rep(0,NLoci)
RPP = rep(0,NLoci)
PBar1 = rep(0,NLoci)
...
Mixed <- function(parents)           { ## This is separate sexes with mixed mating 
 ...
 return(nextgen)            } # end function
  ...
  MixedFamily <- function(parents)           {  ## simulates family-correlated sampling to same extent as above mating design
        # N = number of inds (rows)
        N = dim(parents)[[1]]
        Half = N/2
        Males = seq(1:Half)
